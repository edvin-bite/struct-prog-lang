...
"""
    factor = <number> | <identifier> | "(" expresion ")" | "!" expression | "-" expression
    term = factor { "*"|"/" factor }
...
def parse_factor(tokens):
    ...
    if token["tag"] == "!":
        ast, tokens = parse_expression(tokens[1:])
        # "!" modifies an expression which is a tree
        return {"tag": "!", "value": ast}, tokens
    if token["tag"] == "-":
        ast, tokens = parse_expression(tokens[1:])
        # same situation as "!" for negate
...
def parse_arithmetic_expression(tokens):
    ...
    arithmetic_expression = term { "+"|"-" term }
    """
    # splits into a tree from "+" and "-", this creates the tree branches left and right
...
def parse_relational_expression(tokens):
    ...
    """
    # get first arithmetic expression
    l_node, tokens = parse_arithmetic_expression(tokens)
    while tokens[0]["tag"] in ["<" , ">" , "<=" , ">=" , "==" , "!="]:
        #adds branch after operator / second arithmetic expression
        tag = tokens[0]["tag"]
        right_node, tokens = parse_arithmetic_expression(tokens[1:])
        ast = {"tag": tag, "left": l_node, "right": right_node}
    return ast, tokens
...
# why? to parse relations first and then logicals like AND and OR etc before less than etc
